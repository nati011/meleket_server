...
func New(dialect, connection, defaultUser, defaultPass string, strength int, createDefaultUserIfNotExist bool) (*GormDatabase, error) {
	...
	if err := db.AutoMigrate(new(model.User), new(model.Application), new(model.Message), new(model.NotificationMessage), new(model.Client), new(model.PluginConf)).Error; err != nil {
		return nil, err
	}
